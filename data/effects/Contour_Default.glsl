...
void main( void )
{
	...
	// Detect interior
	bool checkPixelInside = false;
	if( t < border.y+0.5) // Border on the top
		checkPixelInside = CheckPixel( TexCoord.x, b );
	...
	else if( checkPixelInside ) // Fill interior with contour color + alpha
	{
		gl_FragColor = Color;
		gl_FragColor.a = 0.65; // Alpha channel (transparency)
	}
	else
	{
		discard;
